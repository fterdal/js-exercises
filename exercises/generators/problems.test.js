...
describe('Generator Functions & Yield Exercises', () => {
  ...
  describe('rangeGenerator', () => {
    xtest('should generate range with default step', () => {
      const gen = rangeGenerator(1, 5);
      expect([...gen]).toEqual([1, 2, 3, 4, 5]);
    });

    xtest('should generate range with custom step', () => {
      const gen = rangeGenerator(0, 10, 2);
      expect([...gen]).toEqual([0, 2, 4, 6, 8, 10]);
    });

    xtest('should handle negative step', () => {
      const gen = rangeGenerator(5, 1, -1);
      expect([...gen]).toEqual([5, 4, 3, 2, 1]);
    });

    xtest('should handle single value', () => {
  ...
  describe('take', () => {
    xtest('should take first n values', () => {
      const source = numberGenerator(10);
      const gen = take(source, 5);
      expect([...gen]).toEqual([1, 2, 3, 4, 5]);
    });

    xtest('should handle n larger than generator', () => {
  ...
  describe('filter', () => {
    xtest('should filter even numbers', () => {
      const source = numberGenerator(10);
      const gen = filter(source, n => n % 2 === 0);
      expect([...gen]).toEqual([2, 4, 6, 8, 10]);
    });

    xtest('should filter with custom predicate', () => {
      const source = numberGenerator(10);
  ...
  describe('map', () => {
    xtest('should map values', () => {
      const source = numberGenerator(5);
      const gen = map(source, n => n * 2);
      expect([...gen]).toEqual([2, 4, 6, 8, 10]);
    });

    xtest('should map to strings', () => {
      const source = numberGenerator(3);
  ...
  describe('repeat', () => {
    xtest('should repeat value n times', () => {
  ...
  describe('zip', () => {
    xtest('should zip two generators', () => {
    ...
    xtest('should stop when first generator ends', () => {
    ...
    xtest('should stop when second generator ends', () => {
  ...
  describe('flatten', () => {
    xtest('should flatten nested generators', () => {
    ...
    xtest('should handle empty generators', () => {
      function* empty() {}
      function* gen1() { yield 1; yield 2; }
      function* source() {
        yield empty();
